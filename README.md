...
| `--log` | вести журнал, навіть якщо в налаштуваннях його вимкнено |
...
| `--export` | зберегти історію та інвентар у CSV і HTML у теку стану й вийти |
| `--probe-protocol` | перевірити, чи Twitch досі приймає запити програми, і вийти |
...
| `watch_games` | ігри під спостереженням: сповіщення, коли в них з'являється нова кампанія |
| `autostart` | запускатися разом із Windows (запис повертається, якщо зник із реєстру) |
| `check_updates` | раз на запуск питати GitHub, чи є новіші файли |
| `skip_hopeless` | не братися за кампанії, де до кінця не встигнути жодного дропа |
| `progress_style` | `rainbow` — переливчаста смуга прогресу, `state` — колір за станом |
| `theme_preset` | набір кольорів: порожньо — вбудований, `ocean`, `forest`, `ember`, `grape`, `mono` |
| `keep_log` | вести журнал (типово увімкнено) |
| `log_dir` | тека журналу; порожньо — `Документи\TwitchDropFarm` |
| `hint_links` | підказувати про кампанії, яким бракує прив'язки акаунта (для ігор із `priority` і `watch_games`) |
| `discord_webhook` | адреса вебхука Discord для важливих подій (нагороди, застій, втрачена прив'язка, помилки); порожньо — вимкнено |
...
`/watch add|remove <гра>`, `/report [днів]`, `/export`, `/update`, `/menu`, `/help`.
`/report` показує підсумок за 90 днів (можна від 1 до 365), `/update` ставить знайдене оновлення.
Більшість команд доступна й кнопками в `/menu`.
...
history.jsonl    історія дропів (для /report і експорту)
seen-campaigns.json  кампанії, які вже бачили (для watch_games)
theme.json       тема, збережена з вікна
images           кеш картинок нагород
lock.file        захист від двох копій одночасно
browser_profile  профіль браузера для входу
```

Журнал пишеться в `Документи\TwitchDropFarm\log.txt` (або в `log_dir`); якщо теки «Документи»
немає — у теку стану. Експорт (`--export`, `/export`) кладе в теку стану `history.csv`,
`history.html`, `inventory.csv` і `inventory.html`.
