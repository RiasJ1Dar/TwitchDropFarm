...
На Windows — через реєстр (`HKCU\\...\\Run`), а не ярликом у теці
автозавантаження: запис під поточним користувачем не потребує прав
адміністратора, легко читається й знімається, і його видно там, де його
шукають — у диспетчері завдань. Свідомо не чіпаємо `HKLM`.

На Linux і macOS галочка в GUI не керує автозапуском: див.
`docs/autostart-linux-macos.md` (systemd user / LaunchAgent).
"""
...
def is_supported() -> bool:
    """Чи вміє програма сама ставити/знімати автозапуск на цій ОС."""
