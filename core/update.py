...
def write_apply_script() -> Path:
    ...
    body = (
        ...
        # Обмежене очікування, не вічне. 18.08 скрипт завис назавжди: програма
        # перед виходом перезапускала себе сама, ім'я процесу з'являлось знову,
        # і цикл не мав виходу. Саму причину усунено (перезапуск робить лише
        # цей скрипт), але запобіжник лишається: краще спробувати копіювати й
        # чесно впасти, ніж мовчки висіти в пам'яті.
        "set WAIT=0\r\n"
        ":wait_image\r\n"
        "set /a WAIT+=1\r\n"
        "if %WAIT% GTR 30 (\r\n"
        "  echo image still running after %WAIT% checks, trying anyway >> \"%LOG%\"\r\n"
        "  goto copy_start\r\n"
        ")\r\n"
        "ping -n 2 127.0.0.1 >nul\r\n"
