...
# Скільки канал, який двічі не зарахував хвилину, лишається поза добором.
# ⚠️ Півгодини — компроміс. Менше не має сенсу: причина збою (застряглий
# транскодер, проблема на боці Twitch) рідко минає за хвилини. Більше — і ми
# викидаємо канал, який давно полагодився, а на популярних іграх кандидатів
# буває всього кілька. Детектор збою в нас був і раніше, але пам'яті про
# «цей канал уже підводив» не було, і майнер спокійно вертався на нього.
CHANNEL_BENCH_SECONDS = 30 * 60

# Коли кампанію з початим прогресом рятувати поперед усього іншого.
# Порівнюється зі `slack` — у скільки разів часу до кінця більше, ніж треба
# годин перегляду. 2.0 означає «запасу лишилось удвічі», тобто зволікати вже
# ніколи.
#
# ⚠️ Поріг має бути з запасом, а не «коли горить». 31.08 програма чесно
# попередила про ризик у `Anniversary Store #9` — але на той момент треба було
# 43 хвилини, а лишалось 8: рятувати вже не було чого. Дроп згорів на 206/240,
# і разом із ним майже чотири години перегляду.
RESCUE_SLACK = 2.0

# Скільки хвилин до нагороди вважається «вже майже», і канал не міняють.
# ⚠️ Перемикання за десять хвилин до дропа не просто відкладає нагороду — воно
# лишає намайнене висіти й починає копити хвилини деінде. Чверть години —
# компроміс: довше означало б тримати канал, який давно перестав бути
# найкращим вибором.
ALMOST_THERE_MINUTES = 15
...
# Три топіки користувача (дропи, сповіщення, поінти) плюс три точкові на
# активний канал: зміна гри, моменти й рейди.
USER_TOPIC_COUNT = 3 + 3
