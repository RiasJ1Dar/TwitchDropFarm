...
class Identity:
    ...
    async def ensure(self) -> None:
        # Швидкий шлях без замка. `ensure()` кличеться перед кожним запитом до
        # Twitch — це понад тисяча разів на добу лише з циклу перегляду, — і
        # майже завжди все вже готове. Замок у цьому випадку не захищав нічого,
        # зате шикував у чергу паралельні пакети запитів, які інакше йшли б
        # одночасно.
        #
        # Перевірка всередині `_ensure_unlocked` лишається: два виклики можуть
        # проскочити сюди водночас, і тоді другий має побачити, що перший уже
        # все зробив, — тому замок нікуди не подівся, лише перестав бути
        # обов'язковою зупинкою для всіх.
        if self.known:
            self._ready.set()
            return
