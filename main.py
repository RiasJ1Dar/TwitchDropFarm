...
if __name__ == "__main__":
    ...
    async def main() -> int:
        ...
        if not one_shot:
            ...
            # Скрипт підміни чекає саме на це: інакше він не відрізнить
            # «встало» від «нова збірка показує вікно помилки».
            _update.confirm_started()

            # Автозапуск: намір людини живе в налаштуваннях, а не лише в
            # реєстрі, тому зниклий запис можна повернути. Перший запуск після
            # оновлення заразом переносить у налаштування те, що вже стоїть, —
            # інакше в тих, хто вмикав автозапуск давніше, намір лишився б
            # порожнім і відновлювати було б нічого.
            from core import autostart as _autostart

            if _autostart.is_enabled():
                if not settings.autostart:
                    settings.autostart = True
            elif settings.autostart and _autostart.restore(True):
                client.events.log("Автозапуск разом із Windows відновлено.")
